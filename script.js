...
function loadPageContent() {
  ...
  container.innerHTML = `<div class="inputDiv"><h1>Pagination Task</h1><p>Display User details with pagination:</p></div><div class="outputDiv"></div><div class="pagination"></div>`;
  document.body.append(container);

  const content = document.querySelector(".outputDiv");
  for (let k = 0; k < 10; k++) {
    content.innerHTML += `<div class="userInfo"><div class="userName">Name: ${data[k].name}</div><div class="userEmail">Email: ${data[k].email}</div></div>`;
  }

  let noOfPages = Math.ceil(data.length / 10);
  console.log(noOfPages);
  const result = document.querySelector(".pagination");
  for (let i = 1; i <= noOfPages; i++) {
    result.innerHTML += `<button class="btn btn${i}" onclick="changePage(${i})" value="${i}">${i}</button>`;
  }
}
...
function changePage(pageNum) {
  console.log("Hello");
  console.log(document.querySelector(`.btn${pageNum}`).value);

  const content = document.querySelector(".outputDiv");
  content.innerHTML = "";

  for (let j = (pageNum - 1) * 10; j < pageNum * 10; j++) {
    content.innerHTML += `<div class="userInfo"><div class="userName">Name: ${data[j].name}</div><div class="userEmail">Email: ${data[j].email}</div></div>`;
  }
}
